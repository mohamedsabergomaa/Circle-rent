...
ob = ob.replace(
  /const setStatus = \(id: string, status: BookingStatus, message: string\) => \{ const updated = bookings\.map\(item => item\.id === id \? \{ \.\.\.item, status \} : item\); setBookings\(updated\); saveBookings\(updated\); setNotice\(message\); window\.setTimeout\(\(\) => setNotice\(''\), 2600\) \}/,
