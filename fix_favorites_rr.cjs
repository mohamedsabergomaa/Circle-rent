...
fav = fav.replace(
  "import('react').then(React => useEffect(() => { getFavorites().then(setItems).catch(console.error) }, []));",
...
rr = rr.replace(
  "import('react').then(React => React.useEffect(() => { getBookings().then(b => setBooking(b.find(x => x.id === id) || null)).catch(console.error) }, [id]));",

  "useEffect(() => { getBookings().then(b => setBooking(b.find(x => x.id === id) || null)).catch(console.error) }, [id]);",
)

rr = rr.replace(
  "import('react').then(React => useEffect(() => { getBookings().then(b => setBooking(b.find(x => x.id === id) || null)).catch(console.error) }, [id]));",
...
rr = rr.replace(
  ...
  "import { getMyBookings as getBookings, updateBookingStatus }",
)
