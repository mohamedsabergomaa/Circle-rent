...
if (!app.includes('useEffect,')) app = app.replace('import { useState', 'import { useState, useEffect');
...
ss = ss.replace(/const remove = \(id: string\) => \{.*?window\.setTimeout\(\(\) => setNotice\(''\), 2300\) \}/s,
...
ss = ss.replace(/const update = \(name: string\) => \{.*?window\.setTimeout\(\(\) => setNotice\(''\), 2300\) \}/s,
...
pf = pf.replace("const [bookings] = useState(() => getBookings())",
  "const [bookings, setBookings] = useState<MockBooking[]>([]); useEffect(() => { getBookings().then(setBookings).catch(console.error) }, [])");
if(!pf.includes('useEffect')) pf = pf.replace("import { useState }", "import { useState, useEffect }");
