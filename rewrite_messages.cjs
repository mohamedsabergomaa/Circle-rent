...
content = content.replace(
  /const select = \(id: string\) => \{.*?navigate\(\`\/messages\?conversation=\$\{id\}\`, \{ replace: true \}\) \}/g,

  "const select = (id: string) => { setSelectedId(id); setMobileOpen(true); navigate(`/messages?conversation=${id}`, { replace: true }) }",
)

content = content.replace(
  /const send = \(body: string\) => \{ if \(\!selected \|\| \!body\.trim\(\)\) return; .*?saveConversations\(nextConversations\) \}/g,
