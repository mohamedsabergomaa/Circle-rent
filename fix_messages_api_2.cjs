...
c = c.replace(/const \[messages, setMessages\] = useState<Message\[\]>\(\[\]\); useEffect\(\(\) => \{ if \(selectedId\) \{ getMessages\(selectedId\)\.then\(setMessages\)\.catch\(console\.error\) \} \}, \[selectedId\]\); const \[selectedId, setSelectedId\] = useState<string \| null>\(params\.get\('conversation'\)\);/,
...
c = c.replace(/const last: Message \| null = null; \/\/ replaced by API/, "const last = null as Message | null;");
c = c.replace(/const last: Message \| null = null \/\/ replaced by API/, "const last = null as Message | null;");
