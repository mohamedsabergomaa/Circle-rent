...
function load(f) { return fs.readFileSync(f, 'utf8'); }
function save(f, c) { fs.writeFileSync(f, c); }

let c;

// Favorites.tsx
c = load('src/pages/Favorites.tsx');
c = c.replace("import { getFavorites, saveFavorites, type FavoriteItem } from '../lib/mockFavorites'",
  "import { getFavorites, removeFavorite } from '../services/favorites'\nimport type { FavoriteItem } from '../types'");
c = c.replace("const [items, setItems] = useState(() => getFavorites())",
  "const [items, setItems] = useState<FavoriteItem[]>([]); useEffect(() => { getFavorites().then(setItems).catch(console.error) }, [])");
c = c.replace(/const remove = \(id: string\) => \{.*?saveFavorites\(next\) \}/,
...
if(!c.includes('useEffect')) c = c.replace("import { useMemo, useState", "import { useMemo, useState, useEffect");
save('src/pages/Favorites.tsx', c);

// SavedSearches.tsx
c = load('src/pages/SavedSearches.tsx');
c = c.replace("import { getSavedSearches, saveSavedSearches, type SavedSearch } from '../lib/mockSavedSearches'",
  "import { getSavedSearches, deleteSavedSearch, updateSavedSearch } from '../services/savedSearches'\nimport type { SavedSearch } from '../types'");
c = c.replace("const [searches, setSearches] = useState(() => getSavedSearches())",
  "const [searches, setSearches] = useState<SavedSearch[]>([]); useEffect(() => { getSavedSearches().then(setSearches).catch(console.error) }, [])");
c = c.replace(/const removeSearch = \(id: string\) => \{.*?setSearches\(next\)\n  \}/s,
...
c = c.replace(/const applyNameEdit = \(\) => \{\n.*?setEditingId\(null\)\n  \}/s,
...
if(!c.includes('useEffect')) c = c.replace("import { useState }", "import { useState, useEffect }");
save('src/pages/SavedSearches.tsx', c);

// OwnerBookings.tsx
c = load('src/pages/OwnerBookings.tsx');
c = c.replace("import { getBookings, saveBookings, type BookingStatus, type MockBooking } from '../lib/mockBookings'",
...
c = c.replace("const [bookings, setBookings] = useState(() => getBookings())",
  "const [bookings, setBookings] = useState<MockBooking[]>([]); useEffect(() => { getBookings().then(setBookings).catch(console.error) }, [])");
c = c.replace(/const updateStatus = \(id: string, status: BookingStatus, message: string\) => \{\n.*?setBookings\(next\)\n  \}/s,
...
c = c.replace("import { getBookings, type MockBooking } from '../lib/mockBookings'",
  "import { getMyBookings as getBookings } from '../services/bookings'\nimport type { MockBooking } from '../types'");
c = c.replace("const [bookings, setBookings] = useState(() => getBookings())",
  "const [bookings, setBookings] = useState<MockBooking[]>([]); useEffect(() => { getBookings().then(setBookings).catch(console.error) }, [])");
c = c.replace("const [bookings] = useState(() => getBookings())",
  "const [bookings, setBookings] = useState<MockBooking[]>([]); useEffect(() => { getBookings().then(setBookings).catch(console.error) }, [])");
if(!c.includes('useEffect')) c = c.replace("import { useState", "import { useState, useEffect");
...
c = c.replace("import { getBookings, saveBookings } from '../lib/mockBookings'",
  "import { getMyBookings as getBookings, updateBookingStatus } from '../services/bookings'\nimport type { MockBooking } from '../types'");
c = c.replace("const [booking] = useState(() => getBookings().find(item => item.id === id));",
...
c = c.replace(/const submit = \(event: FormEvent\) => \{.*?saveBookings\(updated\); setDone\(true\) \}/s,
...
c = c.replace("import { ensureConversation, getConversations, getMessages, saveConversations, saveMessages, type Conversation, type Message } from '../lib/mockMessages'",
  "import { getConversations, getMessages, sendMessage } from '../services/messages'\nimport type { Conversation, Message } from '../types'");
c = c.replace("import { getBookings } from '../lib/mockBookings'", "import { getMyBookings as getBookings } from '../services/bookings'");
c = c.replace("const [conversations, setConversations] = useState(() => getConversations())",
...
c = c.replace("const [messages, setMessages] = useState(() => getMessages())",
...
c = c.replace(/const messages = getMessages\(\)\.filter\(item => item\.conversationId === conversation\.id\)\n.*?const last = messages\.at\(-1\)/s, "const last = null");
if(!c.includes('useEffect')) c = c.replace("import { useMemo, useRef, useState", "import { useMemo, useRef, useState, useEffect");
