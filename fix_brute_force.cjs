...
brute("src/pages/SavedSearches.tsx", (c) => {
  c = c.replace(
    /const remove = \(id: string\) => \{ const next = items\.filter\(item => item\.id !== id\); setItems\(next\); saveSavedSearches\(next\); setNotice\('تم حذف البحث المحفوظ\.'\); window\.setTimeout\(\(\) => setNotice\(''\), 2300\) \}/,
  ...
  c = c.replace(
    /const update = \(name: string\) => \{ if \(\!editing\) return; const next = items\.map\(item => item\.id === editing\.id \? \{ \.\.\.item, name \} : item\); setItems\(next\); saveSavedSearches\(next\); setEditing\(null\); setNotice\('تم تعديل اسم البحث\.'\); window\.setTimeout\(\(\) => setNotice\(''\), 2300\) \}/,
...
brute("src/pages/OwnerBookings.tsx", (c) => {
  c = c.replace(
    /const updateStatus = \(id: string, status: BookingStatus, message: string\) => \{ const next = bookings\.map\(b => \(b\.id === id \? \{ \.\.\.b, status \} : b\)\); saveBookings\(next\); setBookings\(next\); setNotice\(message\); window\.setTimeout\(\(\) => setNotice\(''\), 3000\) \}/,
...
brute("src/pages/Profiles.tsx", (c) => {
  c = c.replace(
    /const visible = useMemo\(\(\) => bookings\.filter\(item => activeTab === 'bookings' \? \['pending', 'approved', 'active'\]\.includes\(item\.status\) : \['completed', 'cancelled', 'declined'\]\.includes\(item\.status\)\), \[bookings, activeTab\]\)/,
  ...
  c = c.replace(
    /const filtered = useMemo\(\(\) => bookings\.filter\(item => activeTab === 'bookings' \? \['pending', 'approved', 'active'\]\.includes\(item\.status\) : \['completed', 'cancelled', 'declined'\]\.includes\(item\.status\)\), \[bookings, activeTab\]\)/,
...
brute("src/pages/Messages.tsx", (c) => {
  c = c.replace(
    ...
    "const conversation = { id: '', unread: 0, /*",
  )

  c = c.replace(
    "setMobileOpen(true) }, [params])",
    "setMobileOpen(true) }, [params]) /* removed ensureConv */",
  )

  c = c.replace(
    "const last = null; // replaced by API",
    "const last = null as Message | null; // replaced by API",
  )
