...
c = c.replace(/const \[bookings\] = useState\(\(\) => getBookings\(\)\)/g, 
...
c = c.replace(/const updateStatus = \(id: string, status: BookingStatus, message: string\) => \{.*?saveBookings\(next\).*?\}/s,
...
function addUseEffect(path) {
  ...
  if(file.includes('useEffect') && !file.includes('useEffect,') && !file.includes('useEffect }')) {
