...
['src/pages/Favorites.tsx', 'src/pages/Messages.tsx', 'src/pages/MyBookings.tsx', 'src/pages/OwnerBookings.tsx', 'src/pages/RentalReturn.tsx', 'src/App.tsx', 'src/pages/SavedSearches.tsx', 'src/pages/Checkout.tsx', 'src/pages/Profiles.tsx'].forEach(path => {
  if (fs.existsSync(path)) {
    replaceFile(path, 'React.useEffect', 'useEffect');
    replaceFile(path, "import('react').then(React => useEffect(", "useEffect(");
    replaceFile(path, "import('react').then(React => React.useEffect(", "useEffect(");
...
['src/pages/MyBookings.tsx', 'src/pages/RentalReturn.tsx', 'src/pages/Messages.tsx', 'src/pages/OwnerBookings.tsx', 'src/pages/Profiles.tsx'].forEach(path => {
  if (fs.existsSync(path)) {
    replaceFile(path, "import { getBookings", "import { getMyBookings as getBookings");
  }
});
...
bookingsTs = bookingsTs.replace("Omit<MockBooking, 'id' | 'createdAt' | 'status'>", "Omit<MockBooking, 'id' | 'createdAt' | 'status' | 'pickupCode' | 'returnCode'>");
