...
content = content.replace(
  /function ConversationRow\(\{ conversation, active, onClick \}: \{ conversation: Conversation; active: boolean; onClick: \(\) => void \}\) \{ const messages = getMessages\(\)\.filter\(item => item\.conversationId === conversation\.id\); const last = messages\.at\(-1\);/g,
