...
for (let file of glob) {
  ...
  if (
    ...
    content = content.replace(
      /const \[favorites, setFavorites\] = useState\(\(\) => getFavorites\(\)\)/g,
    ...
    content = content.replace(
      /onClick=\{\(\) => removeFavorite\(item\.id\)\}/g,
  ...
  if (content.includes("from '../lib/mockBookings'")) {
    content = content.replace(
      /import \{ getBookings, saveBookings, type BookingStatus, type MockBooking \} from '\.\.\/lib\/mockBookings'/g,

      "import { getBookings, updateBookingStatus } from '../services/bookings'\nimport type { BookingStatus, MockBooking } from '../types'",
    )

    content = content.replace(
      /import \{ getBookings, type MockBooking \} from '\.\.\/lib\/mockBookings'/g,

      "import { getBookings } from '../services/bookings'\nimport type { MockBooking } from '../types'",
    )

    content = content.replace(
      /import \{ getBookings, saveBookings \} from '\.\.\/lib\/mockBookings'/g,
    ...
    if (content.includes("useState<MockBooking[]>(() => getBookings()")) {
      content = content.replace(
        /const \[bookings, setBookings\] = useState<MockBooking\[\]>\(\(\) => getBookings\(\).*\)/g,
      ...
      content = content.replace(
        /const \[bookings, setBookings\] = useState\(\(\) => getBookings\(\).*\)/g,
    ...
    content = content.replace(
      /const b = bookings\.map\(x => x\.id === targetBooking\.id \? \{ \.\.\.x, status: 'return_pending' as const \} : x\)\n      saveBookings\(b\)/g,
  ...
  if (content.includes("from '../lib/mockSavedSearches'")) {
    content = content.replace(
      /import \{ getSavedSearches, saveSavedSearches, type SavedSearch \} from '\.\.\/lib\/mockSavedSearches'/g,

      "import { getSavedSearches, deleteSavedSearch, updateSavedSearch } from '../services/savedSearches'\nimport type { SavedSearch } from '../types'",
    )

    content = content.replace(
      /const \[searches, setSearches\] = useState\(\(\) => getSavedSearches\(\)\)/g,
  ...
  if (content.includes("from '../lib/mockMessages'")) {
    content = content.replace(
      /import \{ ensureConversation, getConversations, getMessages, saveConversations, saveMessages, type Conversation, type Message \} from '\.\.\/lib\/mockMessages'/g,

      "import { getConversations, getMessages, sendMessage } from '../services/messages'\nimport type { Conversation, Message } from '../types'",
    )

    content = content.replace(
      /const \[conversations, setConversations\] = useState\(\(\) => getConversations\(\)\)/g,

      "const [conversations, setConversations] = useState<Conversation[]>([])\n  React.useEffect(() => { getConversations().then(setConversations).catch(console.error) }, [])",
    )

    content = content.replace(
      /const \[messages, setMessages\] = useState\(\(\) => getMessages\(\)\)/g,
