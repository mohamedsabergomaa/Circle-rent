...
pf = pf.replace("const [bookings, setBookings] = useState(() => getBookings())",
  "const [bookings, setBookings] = useState<MockBooking[]>([]); useEffect(() => { getBookings().then(setBookings).catch(console.error) }, [])");
if(pf.includes('useEffect') && !pf.includes('useEffect,')) pf = pf.replace("import { useState }", "import { useState, useEffect }");
...
if(ob.includes('useEffect') && !ob.includes('useEffect,')) ob = ob.replace("import { useMemo, useState", "import { useMemo, useState, useEffect");
...
ms = ms.replace(/const messages = getMessages\(\)\.filter\(item => item\.conversationId === conversation\.id\)\n.*?const last = messages\.at\(-1\)/s, "const last: Message | null = null");
if(ms.includes('useEffect') && !ms.includes('useEffect,')) ms = ms.replace("import { useMemo, useRef, useState", "import { useMemo, useRef, useState, useEffect");
fs.writeFileSync('src/pages/Messages.tsx', ms);

// RentalReturn.tsx
let rr = fs.readFileSync('src/pages/RentalReturn.tsx', 'utf8');
rr = rr.replace("import { getBookings, saveBookings } from '../lib/mockBookings'",
  "import { getMyBookings as getBookings, updateBookingStatus } from '../services/bookings'\nimport type { MockBooking } from '../types'");
rr = rr.replace("const [booking] = useState(() => getBookings().find(item => item.id === id));",
...
rr = rr.replace(/const submit = \(event: FormEvent\) => \{.*?saveBookings\(updated\); setDone\(true\) \}/s,
...
if(rr.includes('useEffect') && !rr.includes('useEffect,')) rr = rr.replace("import { useMemo, useState", "import { useMemo, useState, useEffect");
