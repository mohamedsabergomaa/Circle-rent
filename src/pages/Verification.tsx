  ...
  return <VerificationShell><main className="mx-auto grid max-w-5xl gap-10 px-5 py-10 lg:grid-cols-[.75fr_1.25fr] lg:py-16">
    ...
    <section className="rounded-3xl border border-line bg-white p-6 shadow-sm sm:p-8"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-brand-soft text-brand"><UserRound size={20} /></span><div><h2 className="text-2xl font-black text-ink">تأكيد هويتك</h2><p className="mt-1 text-sm leading-6 text-ink/60">هذه الخطوة مطلوبة قبل استخدام ميزات التأجير أو إدارة الإعلانات.</p></div></div>
      <form onSubmit={submit} className="mt-8 space-y-6"><div><label htmlFor="phone" className="mb-2 block text-sm font-bold text-ink">رقم الهاتف</label><input id="phone" required inputMode="tel" value={phone} onChange={event => setPhone(event.target.value)} placeholder="+20 100 000 0000" className="w-full rounded-xl border border-line px-4 py-3 text-sm text-ink outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/15" /><p className="mt-2 text-xs text-ink/50">سنستخدمه للتحقق والتواصل فقط.</p></div>
        ...
        <div className="flex gap-3 rounded-2xl border border-amber/30 bg-amber/10 p-4 text-xs leading-6 text-ink/70"><LockKeyhole size={18} className="mt-0.5 shrink-0 text-[#9c6514]" />تأكد أن الاسم والصورة في البطاقة واضحان. نقبل JPG وPNG وWebP فقط.</div>
        <button disabled={!canSubmit || submitted} type="submit" className="flex w-full items-center justify-center gap-2 rounded-full bg-brand py-3.5 text-sm font-bold text-cream transition hover:bg-[#4a2650] disabled:cursor-not-allowed disabled:opacity-45">{submitted ? <><Clock3 size={18} /> جارٍ إرسال طلبك...</> : <><Upload size={18} /> إرسال طلب التحقق</>}</button>
      </form>
    </section>
...
export function VerificationPending() {
  ...
  return <VerificationShell><main className="mx-auto flex min-h-[calc(100vh-73px)] max-w-xl items-center px-5 py-12"><section className="w-full rounded-3xl border border-line bg-white p-7 text-center shadow-sm sm:p-10"><div className="relative mx-auto grid h-20 w-20 place-items-center rounded-full bg-amber/15 text-[#a96b12]"><Clock3 size={34} /><span className="absolute -bottom-1 -right-1 grid h-7 w-7 place-items-center rounded-full bg-green text-white"><Check size={16} /></span></div><p className="mt-7 text-xs font-bold tracking-[.14em] text-[#a96b12]">تم استلام الطلب</p><h1 className="mt-3 text-3xl font-black text-ink">حسابك قيد المراجعة</h1><p className="mx-auto mt-4 max-w-md text-sm leading-7 text-ink/60">شكرًا لإرسال بياناتك. يراجع فريق سيركل طلبك عادةً خلال ٢٤ ساعة، وسنرسل إليك إشعارًا فور اكتمال التحقق.</p><div className="mt-8 grid gap-3 rounded-2xl bg-cream p-4 text-right text-sm"><div className="flex items-center gap-3"><span className="grid h-7 w-7 place-items-center rounded-full bg-green text-white"><Check size={15} /></span><span className="font-medium text-ink">تم إرسال رقم الهاتف والمستندات</span></div><div className="flex items-center gap-3"><span className="grid h-7 w-7 place-items-center rounded-full bg-amber/20 text-[#a96b12]"><Clock3 size={15} /></span><span className="font-medium text-ink">مراجعة الهوية قيد التنفيذ</span></div><div className="flex items-center gap-3"><span className="grid h-7 w-7 place-items-center rounded-full bg-white text-ink/35"><ShieldCheck size={15} /></span><span className="text-ink/55">ستُفعّل ميزات الحساب بعد الموافقة</span></div></div><p className="mt-6 text-xs leading-5 text-ink/50">لن تتمكن من الدخول إلى لوحة التحكم أو نشر الإعلانات قبل اعتماد التحقق.</p><button onClick={signOut} className="mt-6 text-sm font-bold text-brand hover:underline">تسجيل الخروج</button></section></main></VerificationShell>
}
