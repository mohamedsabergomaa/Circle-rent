...
export default function AuthLayout({ eyebrow, title, subtitle, variant, children, footer }: AuthLayoutProps) {
    ...
    <div dir="rtl" lang="ar" className="grid min-h-screen bg-cream lg:grid-cols-[1fr_1.04fr]">
      <main className="order-1 flex min-h-screen flex-col bg-cream px-5 py-5 sm:px-10 lg:order-2 lg:px-[clamp(3rem,7vw,8.5rem)] lg:py-9">
        ...
        <div className="mx-auto flex w-full max-w-[455px] flex-1 flex-col justify-center py-10 lg:py-14">
          ...
          <h1 className="editorial-display mt-3 text-4xl leading-[1.15] text-ink sm:text-[2.8rem]">{title}</h1>
      ...
      <aside className="relative order-2 hidden overflow-hidden bg-brand p-12 text-cream lg:order-1 lg:flex lg:min-h-screen lg:flex-col">
        ...
        <div className="relative flex h-full flex-col justify-between">
          ...
          <section className="max-w-lg">
            ...
            <p className="mt-5 max-w-md text-base leading-8 text-cream/72">{signUp ? 'اعرض ما لديك، وحدد سعرك، واجعل الأشياء المفيدة تدور داخل مجتمعك.' : 'استأجر من مجتمعك المحلي بدل شراء ما تحتاجه لمرة واحدة.'}</p>
          </section>
...
function TrustPreview() {
  return <div className="relative mt-12 w-[min(100%,31rem)] rounded-[1.8rem] border border-white/14 bg-white/10 p-4 backdrop-blur-sm">
    <div className="flex items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-cream text-brand shadow-lg"><ShieldCheck size={29} /></div><div><p className="font-bold">تجربة مبنية على الثقة</p><p className="mt-1 text-sm text-cream/65">تحقق بسيط، وتواصل واضح، وتأجير مطمئن.</p></div></div>
    <div className="mt-4 flex items-center justify-between rounded-2xl bg-cream px-4 py-3 text-brand"><span className="flex items-center gap-2 text-sm font-bold"><span className="h-2 w-2 rounded-full bg-brand" /> من جارك القريب</span><BadgeCheck size={19} className="text-green" /></div>
  </div>
}

function ListingPreview() {
  return <div className="relative mt-12 w-[min(100%,31rem)] overflow-hidden rounded-[1.8rem] border border-white/14 bg-cream p-3 text-ink shadow-[0_25px_60px_-25px_rgba(0,0,0,.55)]">
    <div className="flex gap-3"><div className="grid h-20 w-24 place-items-center rounded-2xl bg-amber/20 text-3xl">📷</div><div className="flex-1 py-1"><p className="text-xs font-bold text-brand">تصوير فوتوغرافي</p><p className="mt-1 font-bold">كاميرا سوني ألفا</p><p className="mt-2 text-sm text-ink/55">٣٠٠ ر.س / اليوم</p></div></div>
    <div className="mt-3 flex items-center justify-between rounded-xl bg-brand-soft px-3 py-2.5 text-xs font-bold text-brand"><span className="flex items-center gap-1.5"><MapPin size={14} /> الرياض · النخيل</span><span className="rounded-full bg-white px-2 py-1 text-[11px]">متاح الآن</span></div>
    <div className="absolute -top-3 left-4 rounded-full bg-amber px-3 py-1.5 text-xs font-black text-brand shadow-sm">أضِف إعلانك خلال دقائق</div>
  </div>
}
...
export function TextField({ label, value, onChange, type = 'text', placeholder, optional = false, error }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder: string; optional?: boolean; error?: string }) {
  return <label className="block"><span className="mb-2 flex items-center justify-between text-sm font-bold text-ink">{label}{optional && <span className="text-xs font-medium text-muted">اختياري</span>}</span><input value={value} onChange={event => onChange(event.target.value)} type={type} placeholder={placeholder} className={`w-full rounded-2xl border bg-white px-4 py-3.5 text-sm text-ink outline-none transition placeholder:text-muted/70 ${error ? 'border-rose ring-2 ring-rose/10' : 'border-line focus:border-brand focus:ring-2 focus:ring-brand/10'}`} />{error && <span className="mt-2 block text-xs font-medium text-rose">{error}</span>}</label>
}
