...
export default function RentalReturn() {
  ...
  useEffect(() => {
    getBookings()
      .then((b) => setBooking(b.find((x) => x.id === id) || null))
      .catch(() => {})
  }, [id])
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-cream">
        ...
        <main className="mx-auto max-w-lg px-5 py-24 text-center">
          ...
          <Link
            to="/my-bookings"
            className="mt-5 inline-block font-bold text-brand"
          >
            العودة إلى حجوزاتي
          </Link>
        </main>
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await updateBookingStatus(booking.id, "return_pending")
      setDone(true)
    } catch (e) {
      console.error(e)
    }
  ...
  return (
    <div dir="rtl" lang="ar" className="min-h-screen bg-cream">
      ...
      <main className="mx-auto max-w-5xl px-5 py-8 sm:py-11">
        ...
        >
          <ArrowLeft size={17} /> العودة إلى تفاصيل الحجز
        </Link>
        <div className="mt-6 grid gap-7 lg:grid-cols-[minmax(0,1fr)_19rem]">
          <section>
            <div className="rounded-[1.8rem] border border-line bg-white p-6 sm:p-8">
              ...
              <h1 className="editorial-display mt-2 text-4xl leading-tight text-ink">
                لنُتمّ إرجاع العنصر بسهولة.
              </h1>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-ink/60">
                راجع حالة العنصر قبل الإرجاع وأرسل طلبك للمالك. لن يكتمل التأجير
                إلا بعد تأكيد الاستلام.
              </p>
              <form onSubmit={submit} className="mt-8 space-y-6">
                <section className="rounded-2xl bg-cream p-5">
                  ...
                  <h2 className="mt-2 text-xl font-black text-ink">
                    هل كل شيء جاهز للعودة؟
                  </h2>
                  <div className="mt-5 space-y-3">
                    {[
                      "العنصر بحالته المتفق عليها",
                      "الملحقات الأساسية مرفقة",
                      "لم يتعرض العنصر لضرر جديد",
                      "تم الاتفاق على وقت الإرجاع مع المالك",
                    ].map((label, index) => (
                      <label
                        key={label}
                        className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 text-sm font-bold transition ${
                          checks[index]
                      ...
                      >
                        <input
                          ...
                          type="checkbox"
                          className="h-4 w-4 accent-[#3d8c6c]"
                        />
                        {label}
                        {checks[index] && (
                ...
                <section className="rounded-2xl border border-line p-5">
                  ...
                  <label className="mt-4 block text-sm font-bold text-ink">
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="مثال: سأصل في الموعد المتفق عليه، والملحقات كلها موجودة."
                  ...
                  <div className="mt-5">
                    {file && preview ? (
                      ...
                      <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-line px-4 py-5 text-sm font-bold text-brand transition hover:border-brand hover:bg-brand-soft">
                        <FileImage size={19} /> أضف صورة لحالة العنصر{" "}
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(event) =>
                ...
                <section className="rounded-2xl border border-amber/30 bg-amber/10 p-5">
                  <div className="flex gap-3">
                    ...
                    <p className="text-sm leading-7 text-ink/70">
                      سيصل طلب الإرجاع إلى المالك. احتفظ بمحادثة التأجير لتأكيد
                      مكان ووقت التسليم النهائي.
                    </p>
                  </div>
                  <button
                    disabled={checks.some((value) => !value)}
                    className="mt-5 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-amber text-sm font-black text-brand disabled:cursor-not-allowed disabled:opacity-45"
                  >
                    <PackageCheck size={18} /> إرسال طلب الإرجاع
                  </button>
                </section>
              </form>
            </div>
          ...
          <aside className="space-y-4">
            <section className="overflow-hidden rounded-[1.5rem] border border-line bg-white">
              ...
              <div className="p-5">
                ...
                <div className="mt-5 space-y-3 border-t border-line pt-4 text-sm text-ink/60">
                  ...
                  <p className="flex items-center gap-2">
                    ...
                    {booking.delivery ? "توصيل إلى موقعك" : "استلام شخصي"}
                  </p>
                  <p className="flex items-center gap-2">
                    <Truck size={17} className="text-brand" /> الإرجاع مع{" "}
                    {booking.ownerName}
                  </p>
                </div>
              </div>
      ...
      <main className="mx-auto flex min-h-[calc(100vh-90px)] max-w-xl items-center px-5 py-12">
        <section className="w-full rounded-[2rem] border border-line bg-white p-8 text-center shadow-[0_28px_70px_-44px_rgba(7,92,61,.22)]">
          ...
          <h1 className="editorial-display mt-3 text-4xl leading-tight text-ink">
            بقي تأكيد المالك فقط.
          </h1>
          <p className="mx-auto mt-4 max-w-md text-sm leading-7 text-ink/60">
            تم تسجيل طلب إرجاع <strong>{listingName}</strong>. تواصل مع المالك
            لتأكيد الوقت والمكان.
          </p>
          <div className="mt-8 rounded-2xl bg-cream p-4 text-right text-sm">
            <p className="font-bold text-ink">رقم الحجز</p>
            <p className="mt-1 font-mono text-brand">{bookingId}</p>
          </div>
          <button
            onClick={onView}
            className="mt-7 rounded-xl bg-brand px-6 py-3.5 text-sm font-black text-cream"
          >
