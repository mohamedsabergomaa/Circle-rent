...
function demoResponse(path: string, init: RequestInit): unknown {
  ...
  if (seg[0] === "auth") {
    if (seg[1] === "sign-in") return DEMO_SESSION

    if (seg[1] === "sign-up") return DEMO_SESSION

    if (seg[1] === "forgot-password") return { sent: true }

    if (seg[1] === "reset-password") return { success: true }
  ...
  if (seg[0] === "admin") {
    ...
    if (seg[1] === "users") {
      ...
      if (method === "PUT")
        return {
          id: seg[2],
          status: seg[3] === "suspend" ? "suspended" : "active",
        }
    ...
    if (seg[1] === "listings") {
      ...
      if (method === "PUT")
        return {
          id: seg[2],
          status: seg[3] === "approve" ? "active" : "rejected",
        }
...
export function friendlyError(
  ...
  if (err instanceof ApiError) {
    if (err.offline)
      return "الخدمة غير متاحة حاليًا. تحقق من اتصالك أو حاول لاحقًا."

    if (err.status === 401) {
      if (err.message && err.message !== "Unauthorized") return err.message

      return "غير مصرح لك بتنفيذ هذا الإجراء."
    }

    if (err.status === 403) return "غير مصرح لك بتنفيذ هذا الإجراء."
